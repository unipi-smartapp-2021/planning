...
class RosStpNode():
    ...
    def print(self,ltp_plan, new_figure=False):
        ...
        plt.scatter([pos_x for pos_x, _ in self.trackMap.left_cones], [pos_y for _, pos_y in self.trackMap.left_cones],
                    color='blue', label='Left Cones')
        plt.scatter([pos_x for pos_x, _ in self.trackMap.right_cones], [pos_y for _, pos_y in self.trackMap.right_cones],
    ...
    def run(self):
        ...
        # Create STP object
        self.stp = STP()
        car_pos = self.read_car_location()
        print(car_pos)
        self.stp.set_car_pos_vel(car_pos, (0.1, 0))
